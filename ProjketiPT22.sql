...
--Insertimi i te dhenave ne Tabelen Prind Produkti
 SET IDENTITY_INSERT Furnizuesi on
...
INSERT INTO [dbo].[Produkti]([Produkti_ID],[Emri_Produktit],[FurnizuesiID],[Sasia_Disponueshme],[Cmimi],[Foto],[Renditja_Nivelit],[SasiaPerNjesi],[Njesia_Madhesis],[Zbritja],[Pershkrimi_Produktit],[KategoriaID],[AdminiID]) VALUES (1018,'Solomon Kepuc per Vjaza',5,5,125.25,null,4,2,'P',null,'Waterproof',998,65)
INSERT INTO [dbo].[Produkti]([Produkti_ID],[Emri_Produktit],[FurnizuesiID],[Sasia_Disponueshme],[Cmimi],[Foto],[Renditja_Nivelit],[SasiaPerNjesi],[Njesia_Madhesis],[Zbritja],[Pershkrimi_Produktit],[KategoriaID],[AdminiID]) VALUES (1022,'Solomon Kepuc per Vjaza',8,5,89.36,null,4,2,'P',null,'Waterproof',998,39)
	 ...
	 INSERT INTO [dbo].[InformacionePersonale]([Datelindja],[Shteti],[Qyteti],[Rruga],[KodiPostal],[Emaili],[KonsumatoriID]) Values('1960-09-15','Norvegji','Oslo','Jsw32',461100,'OleOnthwheel@gmail.com',11)
		  ...
		  --Insertmimi i te dhenave ne tablelen [InformacionePersonaleFurnizuesi]
		   ...
		   Insert into [dbo].[InforamcioneFaturimi]([Faturimi_ID],[Data_Skadimit_Karteles],[PIN_Kredit_Kartela],[Numri_Kredit_Kartela],[Id_Kartes],[Data_Fatures],[Adresa_Faturimit]) values(10009,'2024-01-11',8521,32569841,000,'2021-01-10','Montenegro/Plave')
			...
			insert into [dbo].[Kontrollon]([FaturimiID],[KonsumatoriID],[FinancieriID])values(10010,5,8510)

			select * from Kontrollon
			...
			--Insertimi i te dhenave ne tabelen Blerjet
			   ...
			   Insert into [dbo].[Blerjet]([KonsumatoriID],[FaturimiID]) values(10,10009)
				  ...
				  insert into [dbo].[Shporta]([Shporta_ID],[Numri_Produkteve],[Cmimi])values(121,1,89.90)
								  ...
								  insert into[dbo].[Ka_ne_Shport]([KonsumatoriID],[ProduktiID],[ShportaID]) values (11,1020,121)
								 
								 select * from Ka_ne_Shport
								 ...
								 insert into [dbo].[Porosia]([PorosiaID],[Data_Porosis],[Data_Dergeses],[Porosia_Permes],[FaturimiID],[TransportuesiID]) values(0010,'2020-12-24','2020-12-28','Online',10009,18)		
						...
						insert into [dbo].[Detajet_e_Porosis]([Porosia_ID],[ProduktiID],[Sasia],[Zbritja],[Cmimi])values(0008,1000,1,null,10.99)
						...
						insert into [dbo].[Jep](KonsumatoriID,[ShportaID],[Porosia_ID],[Vula_kohore])values(8,120,0009,'05:14')
					...
					--11
					update Furnizuesi
					set Numri_Llogarise=12345522
					where Furnizuesi_ID=8

					--12
					update Furnizuesi
					set Numri_Llogarise=19845522
					where Furnizuesi_ID=9

					--13
					update Transportues
					set Emri_Kompanis='postapackup'
					where Emri_Kompanis='Ak'

					--14
					update Transportues
					set Emri_Kompanis='Korkyra Shipping Ltd'
					where Emri_Kompanis='EDX'
					...
					--19
					update Transportues
					set Xhirollogaria=1230000
					where [Transportuesi_ID]=6

					--20
					update Transportues
					set Xhirollogaria=51510000
					where [Transportuesi_ID]=18
					--21
					update Produkti 
					set Cmimi=Cmimi-(Cmimi*0.05)
					select p.Cmimi,p.Cmimi*5/100,p.Cmimi-(p.Cmimi*5/100) [Cmimi me Zbritje]
					from Produkti p


					select * from Produkti
					--Fshirjen e 10 objekteve (DELETE)
					SET IDENTITY_INSERT Mbikqyersi off
					SET IDENTITY_INSERT Furnizuesi off

					SET IDENTITY_INSERT Produkti on
			
					select * from Transportues

					delete from Furnizuesi where Furnizuesi_ID=1;
					delete from [Kategoria] where Kategoria_ID=888;
					delete from Admini where Admini_Username like 'B20'
					delete from Produkti where Produkti_ID=1000;
					delete from Mbikqyresi where Mbikqyresi_ID=82;
					delete from Transportues where Transportuesi_ID=16;
					delete from Jep where KonsumatoriID=1
					delete from Porosia where PorosiaID=0001
					delete from Shporta where Shporta_ID=111;
					delete from Ka_ne_Shport where KonsumatoriID=1
